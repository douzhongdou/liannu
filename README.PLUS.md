...
| 🔒 **动态锁机制** | 运行时分析文件依赖，自动生成 `task.lock` | 最大化并行度，避免代码冲突 |
...
multi-agent-workflow/
├── 🎛️ workflow/                 # 调度中枢核心
│   ├── 📋 task.json        # 任务单一数据源（核心配置）
│   ├── 🔒 task.lock         # 动态文件锁（运行时自动生成）
...
# 3. 查看任务执行时长
jq '.tasks[] | {id, status, estimated_time}' task.json
```
...
| 调度器不分配任务 | `task.json` 格式错误 / 无空闲 Worker | 检查 JSON 格式，增加 Worker 数量或等待任务完成 |
